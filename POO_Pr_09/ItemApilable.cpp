...
ItemApilable::ItemApilable(unsigned int cuantos) : _numElementos(cuantos) {
    if (cuantos == 0) 
        throw std::out_of_range("[ItemApilable] Error al crear un ItemApilable con ninguna unidad");
    if (cuantos > 64) 
        throw std::out_of_range("[ItemApilable] Error al crear un ItemApilable más de 64 unidades");
}
...
void ItemApilable::incrementaNumElementos(unsigned int cantidad) {
    if (_numElementos + cantidad > 64) 
        throw std::out_of_range("[ItemApilabre::decrementaNumElementos] Un item apilable no puede tener más de 64 unidades");
